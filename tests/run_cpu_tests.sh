...
if [ "${DISABLE_XDIST:-0}" -eq "1" ]; then
    CUDA_VISIBLE_DEVICES="" pytest --cov ignite --cov-report term-missing --cov-report xml -vvv tests "${skip_distrib_opt[@]}" -k "$MATCH_TESTS_EXPRESSION"
else
